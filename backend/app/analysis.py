...
def load_dataframe_from_bytes(data: bytes) -> pd.DataFrame:
    """Load a CSV directly from bytes."""
...
def dataframe_preview(
    ...
    """Return the first n rows as JSON-safe dictionaries."""
    ...
    for col in df.columns:
        ...
        if is_numeric:
            ...
            col_info["stats"] = {
                ...
                "mean": (
                    _clean(round(mean_value, 4))
                    if mean_value is not None
                    else None
                ),
                "median": _clean(series.median()),
                "std": (
                    _clean(round(std_value, 4))
...
def build_ai_context(
    ...
    """Build compact dataset context for Gemini."""
